...
OUT = "charts"
W, H = 880, 470
L, R, T, B = 74, 30, 58, 62          # plot margins
PW, PH = W - L - R, H - T - B
...
def chart_burn():
    ...
    p.append(f'<text x="16" y="{T+PH/2:.0f}" class="axt" text-anchor="middle" '
             f'transform="rotate(-90 16 {T+PH/2:.0f})">ETH burned per day</text>')
...
def chart_issuance():
    ...
    p.append(f'<text x="16" y="{T+PH/2:.0f}" class="axt" text-anchor="middle" '
             f'transform="rotate(-90 16 {T+PH/2:.0f})">Annual issuance (% of ETH supply)</text>')
...
def chart_equilibrium():
    ...
    p.append(f'<text x="{L+PW/2:.0f}" y="{T+PH+42:.0f}" class="axt" text-anchor="middle">'
             'ETH staked</text>')
    p.append(f'<text x="16" y="{T+PH/2:.0f}" class="axt" text-anchor="middle" '
             f'transform="rotate(-90 16 {T+PH/2:.0f})">Total staking yield (APR)</text>')
...
# ------------------------------------------- chart 4: yield IS the staking ratio
def chart_identity():
    """Stake rate and staking yield are not two variables. Issuance yield is a
    closed-form function of the staked balance:  APR = 166.28 / sqrt(S).
    The only free component is execution-layer income, and it has collapsed."""
    lo, hi = 8_000_000, 62_000_000
    ymax = 6.5
    px = lambda s: L + (s - lo) / (hi - lo) * PW
    py = lambda v: T + PH - v / ymax * PH

    # Independently reported (staked ETH, total staking yield) observations.
    # Deliberately NOT our reconstructed series, which is derived from the same
    # formula and would make the fit circular.
    obs = [(13_900_000, 5.80, "Sep 2022 · 5.80%", -13, "middle"),
           (41_100_000, 2.60, "Aug 2026 · 2.60%", 30, "end"),
           (41_900_000, 2.78, "Aug 2026 · 2.78%", -14, "start")]

    p = [head("Stake rate and staking yield are the same variable",
              "The curve is not fitted to anything — it is the protocol's own reward "
              "formula, APR = 166.28 / √S. Dots are independently reported yields.")]
    for v in (0, 1.5, 3.0, 4.5, 6.0):
        p.append(f'<line x1="{L}" y1="{py(v):.1f}" x2="{L+PW}" y2="{py(v):.1f}" class="grid"/>')
        p.append(f'<text x="{L-10}" y="{py(v)+4:.1f}" class="ax" text-anchor="end">{v:.1f}%</text>')
    for s in range(10_000_000, hi, 10_000_000):
        p.append(f'<text x="{px(s):.1f}" y="{T+PH+20:.1f}" class="ax" text-anchor="middle">'
                 f'{s/1e6:.0f}M</text>')
    p.append(f'<text x="{L+PW/2:.0f}" y="{T+PH+42:.0f}" class="axt" text-anchor="middle">'
             'ETH staked</text>')
    p.append(f'<text x="16" y="{T+PH/2:.0f}" class="axt" text-anchor="middle" '
             f'transform="rotate(-90 16 {T+PH/2:.0f})">Staking yield (APR)</text>')

    curve = "M" + "L".join(f"{px(s):.1f},{py(issuance(s)/s*100):.1f}"
                           for s in range(lo, hi + 1, 250_000))
    p.append(f'<g class="c1"><path d="{curve}" class="ln"/></g>')
    p.append(f'<g class="c1"><text x="{px(31_000_000):.1f}" y="{py(issuance(31e6)/31e6*100)-20:.1f}" '
             'class="lab txt" text-anchor="middle">Issuance yield — fixed by the protocol</text></g>')

    for s, v, lab, dy, anc in obs:
        th = issuance(s) / s * 100
        dx = {"middle": 0, "end": -10, "start": 10}[anc]
        p.append(f'<g class="c2"><line x1="{px(s):.1f}" y1="{py(th):.1f}" x2="{px(s):.1f}" '
                 f'y2="{py(v):.1f}" class="dsh"/><circle cx="{px(s):.1f}" cy="{py(v):.1f}" '
                 f'r="5.5" class="dot"/></g>')
        p.append(f'<text x="{px(s)+dx:.1f}" y="{py(v)+dy:.1f}" class="note" '
                 f'text-anchor="{anc}">{lab}</text>')

    p.append(f'<g class="c2"><text x="{px(15_500_000):.1f}" y="{py(5.8)+26:.1f}" '
             'class="lab txt" text-anchor="start">gap = fee income</text></g>')
    p.append(f'<text x="{px(15_500_000):.1f}" y="{py(5.8)+41:.1f}" class="note">'
             '1.34pp in 2022 → 0.03–0.21pp today</text>')

    p.append(f'<text x="{L}" y="{T+PH+56:.0f}" class="note">'
             'Correlation between stake rate and issuance yield is −1 by construction, not by '
             'observation: more stake divides the same reward pool.</text>')
    p.append(f'<text x="{L}" y="{T+PH+70:.0f}" class="note">'
             'Only the fee wedge is a free variable, and it has all but vanished.</text>')
    return svg("".join(p))


# ---------------------------------------- chart 5: what actually rides on the yield
def chart_exposure():
    """Liquid-staking derivatives as a share of collateral in the three largest
    Ethereum lending markets. Figures in USD billions, DefiLlama, 13 Aug 2026."""
    rows = [("SparkLend",   3.571, 2.374 + 0.075),
            ("Aave V3",    14.190, 2.456 + 2.515),
            ("Morpho Blue", 7.966, 0.556 + 0.252)]
    total_tvl = sum(r[1] for r in rows)
    total_lst = sum(r[2] for r in rows)
    rows.append(("All three", total_tvl, total_lst))

    ymax = 100.0
    barh, gap = 34, 62
    x0 = L + 96
    bw = PW - 96
    px = lambda pct: x0 + pct / ymax * bw

    p = [head("A third of the collateral is a staking-yield derivative",
              "wstETH and weETH as a share of total value locked, three largest Ethereum "
              "lending markets.")]
    for v in (0, 25, 50, 75, 100):
        p.append(f'<line x1="{px(v):.1f}" y1="{T+4}" x2="{px(v):.1f}" y2="{T+4+len(rows)*gap:.0f}" '
                 f'class="grid"/>')
        p.append(f'<text x="{px(v):.1f}" y="{T+4+len(rows)*gap+18:.0f}" class="ax" '
                 f'text-anchor="middle">{v}%</text>')
    p.append(f'<text x="{px(50):.0f}" y="{T+4+len(rows)*gap+40:.0f}" class="axt" '
             'text-anchor="middle">Share of protocol TVL held as liquid-staking tokens</text>')

    for i, (name, tvl, lst) in enumerate(rows):
        pct = lst / tvl * 100
        y = T + 14 + i * gap
        cls = "c3" if name == "All three" else "c1"
        p.append(f'<text x="{x0-12:.0f}" y="{y+barh/2+5:.0f}" class="lab" text-anchor="end" '
                 f'fill="var(--ink)">{name}</text>')
        p.append(f'<rect x="{x0:.1f}" y="{y:.0f}" width="{bw:.1f}" height="{barh}" rx="4" '
                 f'fill="var(--rule)"/>')
        fill = "var(--s3)" if cls == "c3" else "var(--s1)"
        p.append(f'<rect x="{x0:.1f}" y="{y:.0f}" width="{px(pct)-x0:.1f}" height="{barh}" '
                 f'rx="4" fill="{fill}"/>')
        p.append(f'<text x="{px(pct)+10:.1f}" y="{y+barh/2+5:.0f}" class="lab" '
                 f'fill="var(--ink)">{pct:.1f}%</text>')
        p.append(f'<text x="{px(pct)+58:.1f}" y="{y+barh/2+5:.0f}" class="note">'
                 f'${lst:.2f}B of ${tvl:.2f}B</text>')

    p.append(f'<text x="{L}" y="{H-30:.0f}" class="note">'
             'Exposure is not the same as dependency: wstETH still dominates plain WETH as '
             'collateral at any positive yield. What breaks at a lower yield is the levered</text>')
    p.append(f'<text x="{L}" y="{H-16:.0f}" class="note">'
             'staking loop, which needs the staking yield to exceed the ETH borrow rate. '
             'Lido alone is 43% of all Ethereum DeFi TVL.</text>')
    return svg("".join(p))
...
if __name__ == "__main__":
    ...
    for name, fn in (("burn-collapse", chart_burn),
                     ...
                     ("yield-identity", chart_identity),
                     ("defi-exposure", chart_exposure)):
